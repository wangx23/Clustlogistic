#' Clustered coefficient logistic regression model with common effects
#'
...
#'
#' @examples
Clogistic1 <- function(indexy, y, z, x, sweights, Ni, betam0, eta00,
                       nu = 1, gam = 3, lam = 0.5, inner_loop= 100, maxiter=1000,
                       tol = 0.001, tol1 = 0.001, tol2 = 0.001)
{
  ...
  for(m in 1:maxiter)
  {
    ...
    for(r in 1:inner_loop)
    {
      ...
      diffinner <- norm(eta_r - eta_cur) + norm(beta_r - beta_cur)
      beta_cur <- beta_r
      eta_cur <- eta_r
      lin <- z %*% eta_cur + Xm %*% beta_cur
      mu <- 1/(1+exp(-lin))
    ...
    ### update deltam
    psim <- D %*% betam + vm/nu
    deltam_new <- t(matrix(sapply(1:nrow(psim),function(xx) scad(psim[xx,],lam,nu,gam)),
                           ncol=nrow(psim)))
  ...
  groupest <- getgroup(t(deltam_new),nr)

  lin <- z %*% eta_r + Xm %*% beta_r
  muest  <- 1/(1+exp(-lin))
  nllvalue <-  -sum(wt* (y * lin - log(1+exp(lin))))

  output <- list(eta = eta_cur, beta = beta_cur, betam = betam,
